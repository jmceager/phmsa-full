...
knitr::opts_knit$set(root.dir = "/Users/jameseager/Documents/pst/phmsa-full")
knitr::opts_chunk$set(echo = FALSE, include = FALSE)
datadir <- "/Users/jameseager/Documents/pst/phmsa-clean/data/"
...
## from that phmsa screenshot lmao 
gtAdd <- tibble(Calendar.Year = rep(2021,10),
                name = seq(1930,2020,10),
                value = c(10134,19981,63970,67474,28600,24133,29523,28489,22732,3628))
...
gt2 <- gt.inc %>%
  ...
  filter(SIGNIFICANT == "YES",
        ...
        # INSTALLATION_DECADE>=1930,
         IYEAR < 2022,
         IYEAR > 2011)%>%
  left_join(filter(gtMileNorm,Calendar.Year == 2021), 
            by = c("INSTALLATION_DECADE" = "name"),
            keep = T) %>%
  group_by(INSTALLATION_DECADE)%>%
  summarise(n = n(),
            miles = mean(value),
...
gtphmsa <- tibble(INSTALLATION_DECADE = unique(gt2$INSTALLATION_DECADE),
                  version = rep("PHMSA", length(unique(gt2$INSTALLATION_DECADE))),
...
#          INSTALLATION_DECADE>=1930,
#          IYEAR !=2022)%>%      
#   count(INSTALLATION_DECADE, CAUSE) %>% view()
...
ggsave("./output/plots/SigInstallDecadeGT2.png", width = wdt, height = hgt, units = "px")
